...
class DeepKalmanFilter(nn.Module):
    ...
    def forward(self, x, n_past=10, n_future=10, mode="train", ss_ratio=0.0):
        ...
        # 在 dataset 中预处理 padding，这里直接获取编码后的尺寸
        # x 输入尺寸: (B, T, 2, 158, 165) -> pad 后 (B, T, 2, 160, 168)
        x = torch.nn.functional.pad(x, pad=(0, 3, 0, 2), mode="constant", value=0)
        image_size = (40, 42)  # CNN 编码后的特征空间分辨率
        ...
        if mode == "train":
            ...
            # 预生成随机数，避免在循环中调用 random.random() 造成 GPU 同步
            use_ss = torch.rand(T, device=device) < ss_ratio if ss_ratio > 0 else None
            ...
            for t in range(T):
                ...
                # 使用预生成的随机数替代 random.random()
                if t > 0 and use_ss is not None and use_ss[t].item():
                    z_t = mu_pt

                # 解码
                x_hat_t = self.decoder(z_t)
                x_hat_t = x_hat_t[:, :, :158, :165]  # 裁掉多余部分，恢复原始分辨率
            ...
            for t in range(n_future):
                ...
                # z_t = mu_pt
                z_t = self.reparameterize(mu_pt, logvar_pt)

                x_hat_t = self.decoder(z_t)
                x_hat_t = x_hat_t[:, :, :158, :165]
